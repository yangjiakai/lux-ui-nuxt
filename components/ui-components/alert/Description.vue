<template>
    <v-alert type="error" variant="tonal" class="mb-4 font-weight-bold">
        <template v-slot:prepend>
            <v-icon class="mt-3"></v-icon>
        </template>
        <h5 class="text-h6 text-capitalize">error</h5>
        <div>This is an error alert — check it out!</div>
    </v-alert>
    <v-alert type="warning" variant="tonal" class="mb-4 font-weight-bold">
        <template v-slot:prepend>
            <v-icon class="mt-3"></v-icon>
        </template>
        <h5 class="text-h6 text-capitalize">warning</h5>
        <div>This is an warning alert — check it out!</div>
    </v-alert>
    <v-alert type="info" variant="tonal" class="mb-4 font-weight-bold">
        <template v-slot:prepend>
            <v-icon class="mt-3"></v-icon>
        </template>
        <h5 class="text-h6 text-capitalize">info</h5>
        <div>This is an info alert — check it out!</div>
    </v-alert>
    <v-alert type="success" variant="tonal" class="mb-4 font-weight-bold">
        <template v-slot:prepend>
            <v-icon class="mt-3"></v-icon>
        </template>
        <h5 class="text-h6 text-capitalize">success</h5>
        <div>This is an success alert — check it out!</div>
    </v-alert>
</template>
